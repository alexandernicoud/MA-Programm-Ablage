...
for L in conv_layers:
    ...
    for i in range(n):
        ...
        im = axes[i].imshow(f, cmap="seismic", vmin=global_wmin, vmax=global_wmax)
...
for idx, L in enumerate(conv_layers):
    ...
    im = ax.imshow(mean_act, cmap="jet", vmin=0, vmax=ACTIVATION_CAP)
    ...
    if idx == 0:
        ...
        for i in range(Fmaps):
            a = acts[0,:,:,i]
            im = axes[i].imshow(a, cmap="jet", vmin=0, vmax=ACTIVATION_CAP)
...
fig, ax = plt.subplots()
im = ax.imshow(heat, cmap="jet", extent=extent, aspect="auto", vmin=0, vmax=1)
...
im = ax[1].imshow(sal, cmap="plasma", vmin=0, vmax=1)
